...
struct FakeRequest {
    let response: Any?
    let responseType: Mint.ResponseType
    let statusCode: Int

    static func find(ofType type: Mint.Method, forPath path: String, in collection: [Mint.Method: [String: FakeRequest]]) -> FakeRequest? {
