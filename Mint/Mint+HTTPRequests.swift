...
public extension Mint {
    ...
    /// Request to the specified path, using the provided parameters.
    ///
    /// - Parameters:
    ...
    @discardableResult
    public func request(_ path: String, method: Mint.Method, parameterType: Mint.ParameterType = .json, parameters: Any? = nil, completion: @escaping (_ result: DataResult) -> Void) -> String {
        return handleDataRequest(method, path: path, cacheName: nil, cachingLevel: .none, responseType: .data, completion: completion)
    }
    
    /// Registers a fake request for the specified path. After registering this, every request (with the same method) to the path, will return the registered response.
    ///
    /// - Parameters:
    ///   - path: The path for the faked request.
    ///   - response: An `Any` that will be returned when a request is made to the specified path.
    ...
    public func fakeRequest(_ path: String, method: Mint.Method, response: Any?, statusCode: Int = 200) {
        registerFake(requestType: method, path: path, response: response, responseType: .data, statusCode: statusCode)
    }
    
    /// Registers a fake request to the specified path using the contents of a file. After registering this, every request (with the same method) to the path, will return the contents of the registered file.
    ///
    /// - Parameters:
    ///   - path: The path for the faked request.
    ///   - fileName: The name of the file, whose contents will be registered as a reponse.
    ///   - bundle: The Bundle where the file is located.
    public func fakeRequest(_ path: String, method: Mint.Method, fileName: String, bundle: Bundle = Bundle.main) {
        registerFake(requestType: method, path: path, fileName: fileName, bundle: bundle)
    }
    
    /// Cancels the request for the specified path. This causes the request to complete with error code URLError.cancelled.
    ///
    /// - ParameterType path: The path for the cancelled request.
    public func cancelRequest(_ path: String, method: Mint.Method) {
        let url = try! composedURL(with: path)
        cancelRequest(.data, requestType: method, url: url)
    }
}
...
public extension Mint {
    ...
    ///
    /// - ParameterType path: The path for the cancelled GET request
...
public extension Mint {
    ...
    ///
    /// - ParameterType path: The path for the cancelled PATCH request.
...
public extension Mint {
    ...
    ///
    /// - ParameterType path: The path for the cancelled PUT request.
...
public extension Mint {
    ...
    ///
    /// - ParameterType path: The path for the cancelled POST request.
...
public extension Mint {
    ...
    ///
    /// - ParameterType path: The path for the cancelled DELETE request.
...
public extension Mint {
    ...
    ///
    /// - ParameterType path: The path for the cancelled image download request.
    ...
    public func fakeImageDownload(_ path: String, image: MintImage?, statusCode: Int = 200) {
