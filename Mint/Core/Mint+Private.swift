...
extension Mint {
        ...
        case .memoryAndFile:
            if let object = cache.object(forKey: destinationURL.absoluteString as AnyObject) {
                ...
                if responseType == .image {
                    returnedObject = MintImage(data: object)
                } else {
    ...
    func registerFake(requestType: Method, path: String, fileName: String, bundle: Bundle) {
        do {
    ...
    func registerFake(requestType: Method, path: String, response: Any?, responseType: ResponseType, statusCode: Int) {
    ...
    func handleJSONRequest(_ requestType: Method, path: String, parameterType: ParameterType?, parameters: Any?, parts: [FormDataPart]? = nil, responseType: ResponseType, completion: @escaping (_ result: JSONResult) -> Void) -> String {
    ...
    func handleDataRequest(_ requestType: Method, path: String, cacheName: String?, cachingLevel: CachingLevel, responseType: ResponseType, completion: @escaping (_ result: DataResult) -> Void) -> String {
    ...
    func handleImageRequest(_ requestType: Method, path: String, cacheName: String?, cachingLevel: CachingLevel, responseType: ResponseType, completion: @escaping (_ result: ImageResult) -> Void) -> String {
        if let fakeRequests = fakeRequests[requestType], let fakeRequest = fakeRequests[path] {
            return handleFakeRequest(fakeRequest, path: path) { _, response, error in
            ...
            if let object = object {
                ...
                return requestData(requestType, path: path, cacheName: cacheName, parameterType: nil, parameters: nil, parts: nil, responseType: responseType) { data, response, error in
                    ...
                    var returnedImage: MintImage?
                    if let data = data, data.count > 0, let image = MintImage(data: data) {
                        switch cachingLevel {
                        case .memory:
    ...
    func requestData(_ requestType: Method, path: String, cacheName _: String?, parameterType: ParameterType?, parameters: Any?, parts: [FormDataPart]?, responseType: ResponseType, completion: @escaping (_ response: Data?, _ response: HTTPURLResponse, _ error: NSError?) -> Void) -> String {
        let requestID = UUID().uuidString
            ...
            let session = self.session.dataTask(with: request) { data, response, error in
                ...
                if let httpResponse = response as? HTTPURLResponse {
                    if httpResponse.statusCode >= 200 && httpResponse.statusCode < 300 {
                        ...
                        connectionError = NSError(domain: Mint.domain, code: errorCode, userInfo: [NSLocalizedDescriptionKey: HTTPURLResponse.localizedString(forStatusCode: httpResponse.statusCode)])
                    }
                }
    ...
    func cancelRequest(_ sessionTaskType: TaskType, requestType: Method, url: URL) {
        let semaphore = DispatchSemaphore(value: 0)
